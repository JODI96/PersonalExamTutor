...
$$\mathbf{h} = \begin{bmatrix} 2 \\ 5 \end{bmatrix} \quad \Rightarrow \quad \mathbf{h}^\top = [2, 5]$$
...
$$\mathbf{b} = \begin{bmatrix} 1 \\ 1 \\ 3 \end{bmatrix} \qquad \mathbf{x} = A^{-1}\mathbf{b} = \frac{1}{3} \begin{bmatrix} 2 & 1 & 2 \\ -2 & 2 & 1 \\ 1 & 2 & -2 \end{bmatrix} \begin{bmatrix} 1 \\ 1 \\ 3 \end{bmatrix}$$
...
where $\mathbf{v}^{(1)} = [-0.2110,\ -0.9775]^\top$ (for $\lambda_1$) and $\mathbf{v}^{(2)} = [-0.9775,\ 0.2110]^\top$ (for $\lambda_2$).
...
$$\mathbf{x} = \begin{bmatrix} 4/3 & 1/3 & -2/3 \\ -1/4 & 0 & 1/4 \end{bmatrix} \begin{bmatrix} 1.8 \\ 3.3 \\ 4.1 \end{bmatrix}$$
...
This is the **least-squares solution** — it minimises $\|A\mathbf{x} - \mathbf{b}\|_2$ since the system is overdetermined (no exact solution exists).
