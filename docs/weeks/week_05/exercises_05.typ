// Echte Kursübungen — Woche 5 (konvertiert am 2026-05-09)
...
Diese Übung basiert z.T. auf dem "Code for San Francisco"-Vortrag "Using Apache Spark 2.0 to Analyze the City of San Francisco's Open Data" (https://youtu.be/K14plpZgy_c) von Sameer Farooqui.

==== Vorbereitung

===== Account-Erstellung und Login

Falls Sie noch keinen Databricks-Account haben, gehen Sie auf https://login.databricks.com/?intent=CE_SIGN_UP und melden Sie sich für die kostenlose "Community Edition" an.

Um einen bestehenden Databricks-Account zu verwenden, loggen Sie sich auf https://community.cloud.databricks.com/login.html ein.

===== Notebook laden

*Download Aufgaben- und Musterlösung-Notebook-Archiv*

Laden Sie die Datei `DatAna_PySpark.dbc` herunter.

*Import in Databricks*

+ Klicken Sie in der Databricks-Weboberfläche im Menü links auf "Workspace".
+ Öffnen Sie das Kontextmenü des Workspace-Eintrags "Home" und wählen Sie daraus "Import".
+ Belassen Sie die Einstellung "Import from:" auf "File".
+ Ziehen Sie die Datei `DatAna_PySpark.dbc` in das vorgesehene Feld oder wählen Sie sie über den "browse"-Link.
+ Schliessen Sie den Import durch den "Import"-Button ab.

*Notebook öffnen*

Im Workspace-Eintrag "Home" haben Sie nun ein neues Verzeichnis `DatAna_PySpark`. Durch Anklicken sehen Sie dessen Inhalt: 2 Databricks-Python-Notebooks. Klicken Sie Notebook `Aufgaben: PySpark mit SQL und DataFrames` an, um es zu öffnen.
...
#block(
  ...
  inset: 8pt,
  radius: 4pt,
  [*Hinweis:* Nicht alle Screenshots im Notebook wurden auf die aktuelle Databricks-Version aktualisiert. Daher kann es sein, dass Darstellung, Job-Anzahl, Stage-Anzahl pro Job, Task-Anzahl pro Stage und Laufzeiten z.T. von den Bildern abweichen.]
)
