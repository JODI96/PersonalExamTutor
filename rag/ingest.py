...
def _collect_files(week_nr: int) -> list[Path]:
    """Collect all indexable files from the three subfolders."""
    root = _week_dir(week_nr)
    files = []
    for sub in ("vorlesung", "uebungen", "loesungen"):
        sub_path = root / sub
        if sub_path.exists():
            files += [f for f in sub_path.iterdir()
                      if f.is_file() and f.suffix.lower() in {".pdf", ".adoc", ".txt", ".md"}]
    return files
...
def ingest_week(week_nr: int, progress=None) -> int:
    ...
    pdfs = _collect_files(week_nr)
    if not pdfs:
        raise FileNotFoundError(
            f"Keine Dateien in {week_path}/vorlesung|uebungen|loesungen gefunden.\n"
            f"Lege deine PDFs in die entsprechenden Unterordner."
        )
